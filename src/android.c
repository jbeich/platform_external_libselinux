...
static void set_policy_index(void)
{
	...
	char safemode[PROP_VALUE_MAX];
	int ret;

	policy_index = 0;

	/* Do not reload from /data policy if in safemode. */
	ret = __system_property_get("persist.sys.safemode", safemode);
	if (ret && (strcmp(safemode, "1") == 0))
		return;
