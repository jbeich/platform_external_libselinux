...
static int compile_regex(struct saved_data *data, spec_t *spec, const char **errbuf)
{
	...
	len = strlen(reg_buf);
	cp = anchored_regex = (char *) malloc(len + 2);
	if (!anchored_regex)
		return -1;
	/* Create ...$ regexp.  */
	...
	*cp = '\0';
	...
	spec->regex = pcre_compile(anchored_regex, PCRE_ANCHORED, &tmperrbuf, &erroff, NULL);
	free(anchored_regex);
	if (!spec->regex) {
		if (errbuf)
			*errbuf=tmperrbuf;
			     ...
			     bool partial)
{
	struct saved_data *data = (struct saved_data *)rec->data;
	spec_t *spec_arr = data->spec_arr;
	int i, rc, file_stem, pcre_options = PCRE_ANCHORED;
	...
	for (i = data->nspec - 1; i >= 0; i--) {
		...
		if ((spec_arr[i].stem_id == -1
			...
			if (compile_regex(data, &spec_arr[i], NULL) < 0)
				goto finish;
			if (spec_arr[i].stem_id == -1)
				rc = pcre_exec(spec_arr[i].regex, NULL, key, strlen(key), 0, pcre_options, NULL, 0);
			else
				rc = pcre_exec(spec_arr[i].regex, NULL, buf, strlen(buf), 0, pcre_options, NULL, 0);
