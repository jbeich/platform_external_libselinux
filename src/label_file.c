	...
	if (best >= 0) {
		if (best)
			selinux_log(SELINUX_INFO,
				    "SELinux:  Matched key %s using alias %s, ctx %s\n",
				    key, aliases[i-1], specs[best]->lr.ctx_raw);
		return &specs[best]->lr;
	}
	return NULL;
}
